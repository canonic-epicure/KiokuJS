Class('KiokuJS.Scope', {
    ...
    has : {
        backend         : {
            ...
            // XXX fetch depedencies from has : { attr : { trait
        ...
        objectsByREFADR : Joose.I.Object
    },
...
//                var 
                ...
                this.nodesByREFADR[ node.getObject().__REFADR__ ] = node
            } else
        ...
        unpinNode : function (node) {
            ...
            if (this.hasOwnID( nodeID )) {
                ...
                delete this.nodesByREFADR[ node.getObject().__REFADR__ ]
            } else
        ...
        // XXX not tested
        includeObjects : function (objectsWithIDs, objectsWithOutIDs, options) {
            options = options || {}
            
            if (options.isShallow == null) options.isShallow = true
            
            
            var nodes   = this.collapse(objectsWithIDs, objectsWithOutIDs, options)
            
            return {
                entries : this.encodeNodes(nodes),
                ids     : Joose.A.map(objectsWithOutIDs, this.objectToID, this)
            }
        }
    },
    ...
    continued : {
        ...
        methods : {
            ...
            // XXX not tested
            animateEntries : function (entries, ids) {
                var me          = this
                
                var nodes       = {}
                
                Joose.A.each(this.backend.decodeEntries(entries), function (node) {
                    nodes[ node.ID ] = node
                })
                ...
                linker.link([], 0).andThen(function () {
                    ...
                    this.CONTINUE.apply(this, Joose.A.map(ids, me.idToObject, me))
                })
            },            
